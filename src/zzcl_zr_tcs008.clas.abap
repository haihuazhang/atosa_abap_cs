CLASS zzcl_zr_tcs008 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
*                                           cx_rap_query_provider.
    ...
    METHODS get_issuecategory IMPORTING io_request  TYPE REF TO if_rap_query_request
                                        io_response TYPE REF TO if_rap_query_response
                              RAISING   cx_rap_query_prov_not_impl
                                        cx_rap_query_provider.
ENDCLASS.
...
CLASS ZZCL_ZR_TCS008 IMPLEMENTATION.


  METHOD get_issuecategory.

    DATA: lt_entity TYPE TABLE OF zr_tcs008.
...
*
    try.
        ...
        "handle exception
    endtry.
    SELECT * FROM zr_tcs006
    WHERE language = @lv_curr_lang
    INTO TABLE @DATA(lt_result).

    lt_entity = VALUE #( FOR result in lt_result ( IssueCategoryCode = result-value_low IssueCategoryText = result-text ) ).
  ...
  METHOD if_rap_query_provider~select.
    TRY.
        CASE io_request->get_entity_id( ).
          ...
          WHEN 'ZR_TCS008'.
            get_issuecategory( io_request = io_request io_response = io_response ).
