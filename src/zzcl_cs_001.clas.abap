...
CLASS ZZCL_CS_001 IMPLEMENTATION.
...
*    /iwbep/cl_mgw_data_util=>orderby( it_order = sort_elements CHANGING ct_data = lt_funcs ).
  ...
  METHOD get_UUID.
    ...
    DATA: lt_entity TYPE TABLE OF zr_scs015.

    SELECT uuid, createdby FROM zr_tcs015 "#EC CI_ALL_FIELDS_NEEDED "#EC CI_NOWHERE
*    WHERE language = @lv_curr_lang
    INTO TABLE @DATA(lt_result).
