CLASS zzcl_zr_scs003 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
*                                           cx_rap_query_provider.
    ...
    METHODS get_brandname IMPORTING io_request  TYPE REF TO if_rap_query_request
                                    io_response TYPE REF TO if_rap_query_response
                          RAISING   cx_rap_query_prov_not_impl
                                    cx_rap_query_provider.
ENDCLASS.
...
CLASS ZZCL_ZR_SCS003 IMPLEMENTATION.


  METHOD get_brandname.

    DATA: lt_entity TYPE TABLE OF zr_scs003.
*
    TRY.
      ...
      CATCH cx_abap_context_info_error.
        "handle exception
    ENDTRY.
    SELECT * FROM zr_scs004
    WHERE language = @lv_curr_lang
    INTO TABLE @DATA(lt_result).

    lt_entity = VALUE #( FOR result IN lt_result ( Code = result-value_low description = result-text ) ).
  ...
  METHOD if_rap_query_provider~select.
    TRY.
        CASE io_request->get_entity_id( ).
          ...
          WHEN 'ZR_SCS003'.
            get_brandname( io_request = io_request io_response = io_response ).
